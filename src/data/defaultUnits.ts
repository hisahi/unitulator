...
const defaultUnits: { [quantityName: string]: (UnitSpecification)[] } = defaultUnitsJson;
...
export const getDefaultUnits = (quantity: Quantity): Unit[] | null => {
  ...
  return defaultUnitsForQuantity.map(unit => unitSpecificationToUnit(unit, UNIT));
};
...
export const getBaseUnitsByQuantity = (): { [quantity: string]: Unit[] } =>
  Object.fromEntries(QUANTITIES.map(quantity => [quantity.name, getBaseUnitsForQuantity(quantity)]));
