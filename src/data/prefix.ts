...
import { Fraction, powerOfTenFraction, fractionToString, multiplyFraction } from '../core/fraction';

const P = (symbol: string, name: string, factor: Fraction): Prefix => ({ symbol, name, factor });
const P10 = (symbol: string, name: string, power: number): Prefix => P(symbol, name, powerOfTenFraction(power));
...
PREFIXES.forEach(prefix => {
  const inverse = PREFIXES.find(candidate =>
