...
const mapUnit = (unit: Unit, group: UnitGroup): UnitOption => (
  { name: unit.name, symbol: unit.symbol, quantity: unit.quantity, group: group.label, custom: !!group.custom }
);

const CUSTOM_UNIT_GROUP = { label: 'main:unitsCustom', units: [], custom: true };
...
const getLocalizedUnitNameOrNull = (name: string | null | undefined): string | null => !name ? null : getLocalizedUnitName(name);

const UnitPicker = ({ unitGroups, quantity, value, onChange, allowCustom }: { unitGroups: UnitGroup[], quantity: Quantity | null, value: UnitSelection | null, onChange: (newValue: UnitSelection) => void, allowCustom: boolean }) => {
  ...
  const unitList: UnitOption[] = unitGroups.map(unitGroup => unitGroup.units.map(unit => mapUnit(unit, unitGroup))).flat();
  const unitTable = Object.fromEntries(unitGroups.map(unitGroup => unitGroup.units).flat().map(unit => [unit.name, unit]));
  const unitOptionTable = Object.fromEntries(unitList.map(unitOption => [unitOption.name, unitOption]));
  const hasGroups = unitGroups.find(unitGroup => !unitGroup.label) == null;

  const optionToString = (unit: UnitOption): string =>
    t('main:unitOptionFormat', { name: getLocalizedUnitName(unit.name!), symbol: unit.symbol });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const doFiltering = (options: UnitOption[], state: any): UnitOption[] =>
    options.filter(option => option.custom || caseFold(optionToString(option)).includes(caseFold(state.inputValue)));
  ...
  if (allowCustom) {
    ...
    unitList.push({ name: null, symbol: null, quantity: null, label: t(CUSTOM_UNIT_GROUP.label), group: CUSTOM_UNIT_GROUP.label, custom: true });
  }
  ...
  return (
    <>
      <Autocomplete
        ...
        renderOption={(props, unit) => <li {...props}>{unit.label || optionToString(unit)}</li>}
        renderInput={(params) => <Tooltip placement="top" enterDelay={500} title={selectValue?.label ?? getLocalizedUnitNameOrNull(selectValue?.name) ?? ''}><TextField {...params} label={t('main:unit')} /></Tooltip>}
      />
      {
